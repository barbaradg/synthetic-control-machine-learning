...
#reg = xgb.XGBRegressor(n_estimators=100, learning_rate=0.02, max_depth=4, min_child_weight=1, gamma=0.4,subsample=0.5, colsample_bytree=0.6)
...
np.round(BOXPLOTS4E.groupby('MODELO').agg(MAPE_E=('MAPE_E','mean'), WMAPE_E=('WMAPE_E','mean'), RMSE_E=('RMSE_E','mean')),2)

np.round(BOXPLOTS4T.groupby('MODELO').agg(MAPE_T=('MAPE_T','mean'), WMAPE_E=('WMAPE_T','mean'), RMSE_T=('RMSE_T','mean')),2)
...
# plot_search_results(grid_lr)

#print(grid_lr.best_params_) #500 estimators mejor resultado, sin depth
...
np.round(BOXPLOTS5E.groupby('MODELO').agg(MAPE_E=('MAPE_E','mean'), WMAPE_E=('WMAPE_E','mean'), RMSE_E=('RMSE_E','mean')),2)

np.round(BOXPLOTS5T.groupby('MODELO').agg(MAPE_T=('MAPE_T','mean'), WMAPE_T=('WMAPE_T','mean'), RMSE_T=('RMSE_T','mean')),2)

BOXPLOTS=pd.concat([BOXPLOTS4T,BOXPLOTS5T], axis=0)
...
BOXPLOTS4T.to_csv('xgcs.csv')
BOXPLOTS5T.to_csv('rfcs.csv')